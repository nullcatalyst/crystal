...
#include "SDL.h"
#include "examples/depth_test/depth_test_view.hpp"
...
int main(int argc, char* argv[]) {
  ...
  auto window   = create_window<Context>("render texture (vulkan)", 1280, 720);
  auto ctx      = Context(Context::Desc{
      /* .window                   = */ window,
      /* .application_name         = */ "render texture",
  ...
  auto window   = create_window<Context>("render texture (metal)", 1280, 720);
  auto ctx      = Context(Context::Desc{
      /* .window = */ window,
  });
...
#error No crystal backend chosen.
  ...
  // window is destroyed.
  {
    auto view = RenderTextureView<Context>(ctx, angle);
