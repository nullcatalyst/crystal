...
int main(int argc, char **argv){
  ...
  static struct s_options options[] = {
    ...
    {OPT_FLAG, "X", (char*)&cppFlag, "Output C++ files.  (*.cpp and *.hpp)"},
  ...
  */
  ...
  if( rpflag ){
    ...
    ** generated parser tables smaller. */
    ...
    ReportTable(&lem, mhflag, sqlFlag, cppFlag);

    /* Produce a header file for use by the scanner.  (This step is
    ** omitted if the "-m" option is used because makeheaders will
    ** generate the file for us.) */
    if( !mhflag ) ReportHeader(&lem, cppFlag);
  }
  if( statistics ){
...
void print_stack_union(
  ...
  int cppFlag                 /* True if outputting C++ files */
){
...
void ReportTable(
  ...
  int cppFlag     /* Generate *.cpp instead of *.c files */
){
  FILE *out, *in, *sql;
  char line[LINESIZE];
  int  lineno;
  ...
  in = tplt_open(lemp);
  if( in==0 ) return;
  out = file_open(lemp,cppFlag?".cpp":".c","wb");
  if( out==0 ){
    fclose(in);
    return;
  }
  ...
  */
  ...
  print_stack_union(out,lemp,&lineno,mhflag,cppFlag);
...
void ReportHeader(struct lemon *lemp, int cppFlag)
{
  FILE *out, *in;
  const char *prefix;
  char line[LINESIZE];
  ...
  in = file_open(lemp,cppFlag?".hpp":".h","rb");
  if( in ){
    int nextChar;
  ...
  out = file_open(lemp,cppFlag?".hpp":".h","wb");
  if( out ){
    for(i=1; i<lemp->nterminal; i++){
